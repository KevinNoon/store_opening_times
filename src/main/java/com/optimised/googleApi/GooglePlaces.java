...
public class GooglePlaces {
  ...
  public void savePlaceDetails(Place place) {
    log = LogManager.getLogger(GooglePlaces.class);
...
//    Place placeDB = placeService.findByPlaceId(place.getPlaceId());
    ...
    if (storeNo != null) {
      ...
      CheckForExceptionTime(place.getName(), storeNo, place.getMonOpen(), place.getMonClose(),
          coreTimes.getMonOpen(), coreTimes.getMonClose(), 1); //Monday = DayOfWeek 1
      CheckForExceptionTime(place.getName(), storeNo, place.getTueOpen(), place.getTueClose(),
          coreTimes.getTueOpen(), coreTimes.getTueClose(), 2); //Tuesday = DayOfWeek 2
      CheckForExceptionTime(place.getName(), storeNo, place.getWedOpen(), place.getWedClose(),
          coreTimes.getWedOpen(), coreTimes.getWedClose(), 3); //Wednesday = DayOfWeek 3
      CheckForExceptionTime(place.getName(), storeNo, place.getThuOpen(), place.getThuClose(),
          coreTimes.getThuOpen(), coreTimes.getThuClose(), 4); //Thursday = DayOfWeek 4
      CheckForExceptionTime(place.getName(), storeNo, place.getFriOpen(), place.getFriClose(),
          coreTimes.getFriOpen(), coreTimes.getFriClose(), 5); //Friday = DayOfWeek 5
      CheckForExceptionTime(place.getName(), storeNo, place.getSatOpen(), place.getSatClose(),
          coreTimes.getSatOpen(), coreTimes.getSatClose(), 6); //Saturday = DayOfWeek 6
      CheckForExceptionTime(place.getName(), storeNo, place.getSunOpen(), place.getSunClose(),
  ...
  private Void CheckForExceptionTime(String name, Integer storeNo, LocalTime pOpen, LocalTime pClose,
    ...
    if (hoursChanged) {
      ...
      log.info("Exception added for store No " + et.getStoreNo());
